...
public class Miner implements Observer {
  ...
  static {
    ...
    options.addOption("v", VOTING_OPTION, true,
        "Comma separated list of BIP-9 soft fork voting bits to set in mined blocks.");
    options.addOption("s", SCRIPT_OPTION, true,
        "Add an optional script to the generated coinbase transaction.  Can be used to 'sign' mined coinbase.");
   ...
   * @param nThread
   *                 The number of solver threads to use. Will default to the
   *                 number of
   *                 available CPUs.
   */
  ...
  public void update(Observable o, Object arg) {
    ...
    if (n == Notification.SYSTEM_ERROR) {
      ...
      accepted++;
      Console
          .output(String.format("@|bold,white Accepted block %d of %d|@ @|bold,green (yay!!!)|@", accepted, attempted));
  ...
  protected void report() {
    if (lastWorkTime > 0L) {
      ...
      Console.output(
          String.format("%d cycles, %d solutions, %d errors, %.2f kilocycles/sec", currentCycles, currentSolutions,
              currentErrors, speed));
    }
